...
def permit_allow(
    ...
    Usage:
      harbour permit allow <identity> <server> [--tool PATTERN] [ARG_POLICIES...]

    Examples:
      harbour permit allow agent filesystem
      harbour permit allow agent filesystem --tool "read_*" "path=/home/user/**"
      harbour permit allow agent db --tool "query" "sql=re:^SELECT.*" "db=production"
    """
